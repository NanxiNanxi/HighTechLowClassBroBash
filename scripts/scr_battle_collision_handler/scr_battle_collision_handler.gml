switch (myStatus){
	...
	if(onTheGround){
		if(!isDucking){
			...
			else if(attack_type = ATTACK_STANDING_LOW_KICK){
				if(image_index>=1) and (image_index<=3){
					scr_create_attack(self,1,obj_standing_lk_hitbox)
	...
	if(attack_type = ATTACK_CROUCH_LOW_KICK){
				if(image_index>=0) and (image_index<=3){
					scr_create_attack(self, 1, obj_crouch_lk_hitbox)
...
if(!onTheGround){
	...
	if(attack_type = ATTACK_JUMP_PUNCH){
		if(image_index>=0) and (image_index<=2){
					scr_create_attack(self,1,obj_punch_hitbox)
	...
	if(attack_type = ATTACK_JUMP_LOW_PUNCH){
		if(image_index>=0) and (image_index<=2){
					scr_create_attack(self,1,obj_low_punch_hitbox)
