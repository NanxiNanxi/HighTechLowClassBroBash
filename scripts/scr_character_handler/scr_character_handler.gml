switch(myStatus){
	case STATE_UNLOCK:
		...
		if (onTheGround){
			...
			if(isCrouching){
					...
					if(max(keyboard_check_pressed(K_BLOCK),0,C_BLOCK_PRESSED)==1) {
						...
						audio_play_sound(bgm_realBlock,1000,false)
		...
		if (max(keyboard_check_pressed(K_PUNCH),0,C_HIGH_PUNCH_PRESSED)==1 && !isCrouching){
		...
		else if (max(keyboard_check_pressed(K_LOW_KICK),0,C_LOW_KICK_PRESSED)==1 && !isCrouching){
			scr_set_attack(ATTACK_STANDING_LOW_KICK);
		}
		else if (max(keyboard_check_pressed(K_LOW_PUNCH),0,C_LOW_PUNCH_PRESSED)==1 && onTheGround){
			...
			if(max(keyboard_check_pressed(K_JUMP_KICK),0,C_JUMP_KICK_PRESSED)==1){
			...
			else if(max(keyboard_check_pressed(K_PUNCH),0,C_HIGH_PUNCH_PRESSED)==1 && !onTheGround) {
				scr_set_attack(ATTACK_JUMP_PUNCH)
				var direct = (myEnemy.x>x)?1:-1
				horSpeed = 5*direct
				isJumpAttack = true;
				...
				scr_set_attack(ATTACK_JUMP_LOW_PUNCH)
				var direct = (myEnemy.x>x)?1:-1
				horSpeed = 5*direct
				isJumpAttack = true;
